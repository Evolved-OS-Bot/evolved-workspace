...
def make_app(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("MILESTONE_REFERRAL_SECRET", "milestone-test-secret")
...
def test_educational_intelligence_status_is_protected_and_shadow_only(
    monkeypatch, tmp_path
):
    app = make_app(monkeypatch, tmp_path)
    client = app.test_client()
    path = "/api/v1/educational-intelligence/surveillance/status"
    assert client.get(path).status_code == 401
    response = client.get(path, headers={"X-Hub-Secret": "test-secret"})
    assert response.status_code == 200
    payload = response.get_json()
    assert payload["mode"] == "shadow"
    assert payload["promotion_authority"] == "none"
    assert payload["discord_notification"] == {
...
def test_conversation_clearance_endpoints_are_protected_and_shadow_only(
    monkeypatch,
    tmp_path,
):
    app = make_app(monkeypatch, tmp_path)
    ...
    headers = {"X-Hub-Secret": "test-secret"}
    payload = {
        "source_run_id": "clearance-app-run-1",
        ...
        "rows": [
            {
                ...
                "contact_name": "Protected Person",
                "channel": "SMS",
                "latest_inbound_message_id": "message-1",
                "latest_inbound_at": "2026-08-10T00:00:00+00:00",
                "latest_inbound_excerpt": "Protected message text",
                "message_history_complete": True,
                "classification": {
                    "category": "routine_response",
                    "action": "Reply in GHL",
                    "source": "model",
    ...
    assert client.post(
        "/api/v1/conversation-clearance/observations", json=payload
    ).status_code == 401
    accepted = client.post(
        "/api/v1/conversation-clearance/observations",
        headers=headers,
        json=payload,
    )
    assert accepted.status_code == 202

    assert client.get("/api/v1/conversation-clearance/cases").status_code == 401
    protected = client.get(
        "/api/v1/conversation-clearance/cases", headers=headers
    ).get_json()
    assert protected["mode"] == "shadow"
    assert protected["write_gates"] == {
        "assignment": False,
        "message": False,
    }
    assert protected["secondary_task_creation"] == "prohibited"
    assert protected["cases"][0]["contact_id"] == "contact-1"

    public_shape = client.get(
        "/api/v2/reporting/conversation-clearance", headers=headers
    ).get_json()
    assert public_shape["identified_content"] is False
    rendered = str(public_shape)
    assert "contact-1" not in rendered
    assert "Protected Person" not in rendered
    assert "Protected message text" not in rendered

    blocked_assignment = client.post(
        "/api/v1/conversation-clearance/assign-unassigned",
        headers=headers,
        json={
            "conversation_id": "conversation-1",
    ...
    queue = client.get(
        "/api/v1/conversation-clearance/queue",
        headers=headers,
    ).get_json()
    assert queue["changed_since_delivery"] is True
    assert queue["secondary_task_creation"] == "prohibited"
    acknowledged = client.post(
        "/api/v1/conversation-clearance/delivery-ack",
        headers=headers,
        json={
            "channel": "admin_exception_digest",
    ...
    assert repeated["changed_since_delivery"] is False


def test_staff_bonus_endpoints_are_protected_and_eligibility_is_governed(
    monkeypatch,
    tmp_path,
):
    app = make_app(monkeypatch, tmp_path)
    ...
    assert client.get("/api/v2/reporting/staff-bonus?month=2026-08").status_code == 401
    report = client.get(
        "/api/v2/reporting/staff-bonus?month=2026-08",
        headers=headers,
    )
    assert report.status_code == 200
    assert report.get_json()["available"] is False

    eligibility = client.post(
        "/api/v2/reporting/staff-bonus/eligibility",
        headers=headers,
        json={
            "staff_name": "Nora Silva",
            "effective_from": "2026-08-01",
            "agreement_reference": "signed-doc-1",
            "approved_by": "Peter Brown",
        },
    )
    assert eligibility.status_code == 202
    assert eligibility.get_json()["status"] == "accepted"

    csv_report = client.get(
        "/api/v2/reporting/staff-bonus.csv?month=2026-08",
        headers=headers,
    )
    assert csv_report.status_code == 200
    assert csv_report.mimetype == "text/csv"
    assert b"governed_event_id" in csv_report.data


def test_milestone_referral_attribution_is_protected_and_idempotent(
    monkeypatch,
    tmp_path,
):
    app = make_app(monkeypatch, tmp_path)
    client = app.test_client()
    payload = {
        "referrer_contact_id": "member-1",
        "referred_contact_id": "friend-1",
        "friend_index": 2,
        "outcome": "created",
    }

    assert client.post(
        "/api/v2/reporting/milestone-referrals",
        json=payload,
    ).status_code == 401
    accepted = client.post(
        "/api/v2/reporting/milestone-referrals",
        headers={"X-Milestone-Referral-Secret": "milestone-test-secret"},
        json=payload,
    )
    assert accepted.status_code == 202
    assert accepted.get_json()["status"] == "accepted"
    assert accepted.get_json()["mode"] == "shadow"
    assert accepted.get_json()["publication_impact"] == "none"
    assert accepted.get_json()["workbook_write"] is False

    controlled_test = client.post(
        "/api/v2/reporting/milestone-referrals",
        headers={"X-Milestone-Referral-Secret": "milestone-test-secret"},
        json={
            **payload,
            "referred_contact_id": "controlled-friend-1",
            "test_mode": True,
        },
    )
    assert controlled_test.status_code == 202
    assert controlled_test.get_json()["status"] == "controlled_test"
    assert controlled_test.get_json()["persisted"] is False
    ...
    self_referral = client.post(
        ...
        json={
            **payload,
            "referred_contact_id": "member-1",
        },
    )
    assert self_referral.status_code == 400
    assert "cannot refer themselves" in self_referral.get_json()["error"]
...
def test_dashboard_keeps_attendance_data_quality_out_of_ceo_attention_count(
    monkeypatch,
    tmp_path,
):
    app = make_app(monkeypatch, tmp_path)
    ...
    module = importlib.import_module("operating_data_hub.app")
    original_latest_snapshot = module.service.store.latest_snapshot

    def latest_snapshot(source):
        if source == "strength_assessment_attendance":
            return {"payload": {}}
        return original_latest_snapshot(source)

    monkeypatch.setattr(
        module.service.store, "latest_snapshot", latest_snapshot
    )
    monkeypatch.setattr(
        module.service,
        "sa_attendance_state",
        lambda: {"summary": {}, "exceptions": [{} for _ in range(48)]},
    )

    dashboard = client.get("/api/v1/dashboard", headers=headers).get_json()
    summary = dashboard["system_summary"]
    assert summary["decision_waiting_count"] == 1
    assert summary["data_quality_exception_count"] == 48
...
def test_roster_candidate_ingestion_populates_acceptance_comparison(
    ...
    assert dashboard["roster_candidate"]["added_clients"][0]["client_name"] == "member@example.com"
    ceo = client.get("/api/v1/ceo-report", headers=headers).get_json()
