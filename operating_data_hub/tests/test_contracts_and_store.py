...
def conversation_clearance_payload(
    *,
    run_id="clearance-run-1",
    observed_at="2026-08-10T01:00:00+00:00",
    inbound_id="inbound-1",
    outbound_at=None,
    outbound_automated=None,
):
    return validate_conversation_clearance(
        {
            "schema_version": 1,
            ...
            "rows": [
                {
                    ...
                    "unread": False,
                    "latest_inbound_message_id": inbound_id,
                    "latest_inbound_at": "2026-08-10T00:00:00+00:00",
                    "latest_inbound_excerpt": "Please change my booking",
                    "latest_outbound_message_id": (
                        "outbound-1" if outbound_at else None
                    ),
                    "latest_outbound_at": outbound_at,
                    "latest_outbound_is_automated": outbound_automated,
...
def test_conversation_clearance_is_idempotent_and_keeps_protected_detail(
    tmp_path,
):
    store = HubStore(f"sqlite:///{tmp_path / 'hub.db'}")
    payload = conversation_clearance_payload()

    first = store.accept_conversation_clearance(payload)
    replay = store.accept_conversation_clearance(payload)

    assert first["cases_created"] == 1
    assert replay["cases_created"] == 0
    assert replay["events_appended"] == 0
    identified = store.conversation_clearance_cases(identified=True)
    share_safe = store.conversation_clearance_cases(identified=False)
    assert identified[0]["contact_id"] == "contact-1"
    assert identified[0]["excerpt"] == "Please change my booking"
    assert "contact_id" not in share_safe[0]
    assert "excerpt" not in share_safe[0]


def test_conversation_clearance_read_flag_does_not_close_and_reply_does(
    tmp_path,
):
    store = HubStore(f"sqlite:///{tmp_path / 'hub.db'}")
    store.accept_conversation_clearance(conversation_clearance_payload())
    assert store.conversation_clearance_cases(identified=True)[0][
        "state"
    ] in {"open", "overdue"}

    store.accept_conversation_clearance(
        conversation_clearance_payload(
            run_id="clearance-run-2",
            observed_at="2026-08-10T02:00:00+00:00",
            outbound_at="2026-08-10T01:30:00+00:00",
            outbound_automated=False,
        )
    )
    assert store.conversation_clearance_cases(identified=True)[0][
        "state"
...
def test_conversation_clearance_new_inbound_preserves_prior_cycle(tmp_path):
    store = HubStore(f"sqlite:///{tmp_path / 'hub.db'}")
    store.accept_conversation_clearance(conversation_clearance_payload())
    store.accept_conversation_clearance(
        conversation_clearance_payload(
    ...
    rows = store.conversation_clearance_cases(identified=True)
    assert len(rows) == 2
    assert {row["state"] for row in rows} & {"open", "overdue"}
    assert "reopened" in {row["state"] for row in rows}


def test_failed_clearance_snapshot_does_not_replace_latest_complete(tmp_path):
    store = HubStore(f"sqlite:///{tmp_path / 'hub.db'}")
    store.accept_conversation_clearance(conversation_clearance_payload())
    failed = validate_conversation_clearance(
        {
            "source_run_id": "clearance-run-failed",
            "observed_at": "2026-08-10T03:00:00+00:00",
            "status": "failed",
            "complete": False,
            "pages": 0,
            "error_code": "http_500",
    ...
    store.accept_conversation_clearance(failed)

    assert store.latest_snapshot("conversation_clearance")["payload"][
        "source_run_id"
    ] == "clearance-run-1"
    assert store.conversation_clearance_summary()["extraction"][
        "status"
    ] == "failed"


def test_conversation_exception_queue_suppresses_unchanged_delivery(tmp_path):
    store = HubStore(f"sqlite:///{tmp_path / 'hub.db'}")
    store.accept_conversation_clearance(conversation_clearance_payload())

    preview = store.conversation_delivery_preview(
        channel="admin_exception_digest",
        identified=True,
    )
    assert preview["changed_since_delivery"] is True
    assert preview["secondary_task_creation"] == "prohibited"
    assert preview["record_count"] == 1

    store.acknowledge_conversation_delivery(
        channel="admin_exception_digest",
        queue_fingerprint=preview["queue_fingerprint"],
    )
    repeat = store.conversation_delivery_preview(
        channel="admin_exception_digest",
        identified=True,
    )
    assert repeat["changed_since_delivery"] is False
    ...
    changed = store.conversation_delivery_preview(
        channel="admin_exception_digest",
        identified=True,
    )
...
def test_roster_candidate_compares_exact_identities_without_cutover(tmp_path):
    ...
    assert comparison["added_clients"] == [
        {
            "client_name": "added@example.com",
            "email": "added@example.com",
            "accepted_services": [],
            "candidate_services": ["PT"],
        }
    ]
    assert comparison["removed_clients"] == [
        {
            "client_name": "removed@example.com",
            "email": "removed@example.com",
            "accepted_services": ["PT"],
            "candidate_services": [],
        }
    ]
