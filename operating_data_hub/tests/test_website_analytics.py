...
def test_cutover_periods_are_exact_and_only_appear_when_complete():
    reporting_start = date(2026, 8, 10)

    assert website_v2_cutover_periods(
        None, "2026-08-17T01:00:00Z"
    ) == {}
    assert website_v2_cutover_periods(
        reporting_start, "2026-08-16T01:00:00Z"
    ) == {}
    assert website_v2_cutover_periods(
        reporting_start, "2026-08-17T01:00:00Z"
    ) == {
        "website_v2_7d": (date(2026, 8, 10), date(2026, 8, 16))
    }
    assert website_v2_cutover_periods(
        reporting_start, "2026-09-07T01:00:00Z"
    ) == {
        "website_v2_7d": (date(2026, 8, 10), date(2026, 8, 16)),
        "website_v2_14d": (date(2026, 8, 10), date(2026, 8, 23)),
        "website_v2_28d": (date(2026, 8, 10), date(2026, 9, 6)),
    }


def test_marketing_snapshot_queries_exact_cutover_window():
    reader = FakeReader()
    snapshot = build_website_marketing_snapshot(
        reader=reader,
        subscriber_submissions=[
            _submission(
                "s-cutover",
                "contact-cutover",
                "2026-08-11T10:00:00+10:00",
            )
        ],
        analytics_started_on=date(2024, 10, 23),
    ...
    cutover = snapshot["periods"]["website_v2_7d"]
    assert cutover["period_start"] == "2026-08-10"
    assert cutover["period_end"] == "2026-08-16"
    assert cutover["new_subscribers"] == 1
    assert cutover["visitors"] == 40
    assert (date(2026, 8, 10), date(2026, 8, 16)) in reader.calls


def test_day_58_matures_first_28_day_booking_cohort():
    result = subscriber_assessment_booking_periods(
        subscriber_submissions=[
            _submission(
                "s-last-day",
        ...
        assessment_appointments=[],
        observed_at="2026-10-06T23:00:00Z",
        additional_periods={
            "website_v2_28d": (
                date(2026, 8, 10),
                date(2026, 9, 6),
    ...
    assert result["periods"]["website_v2_28d"][
        "open_booking_window"
    ] == 0
