...
CORE_METRIC_DEFINITIONS = (
    ...
    },
    {
        ...
        "source_authority": {
            ...
            "completion": "GHL four-control human completion event",
        },
        "numerator_definition": (
            "accepted four-control completion event"
        ),
        "denominator_definition": (
            "Not published until an exact period eligibility-state event is "
            "available; appointment date is not substituted."
        ),
        "inclusion_rules": [
            "eligible Pre-Qualified-or-later WARM stage",
            "non-empty governed Pre-qual Summary",
            "human SA: Pre-Qual Completed By",
            "Brisbane ISO 8601 SA: Pre-Qual Completed At",
        ],
        "exclusion_rules": [
        ...
        "period_semantics": (
            "Completed At in Brisbane-local period; never appointment date "
            "or mutable opportunity update time"
        ),
        "minimum_freshness": {"ghl_hours": 14},
        "owner": "Peter Brown",
        "approval_state": "approved_shadow",
...
class ReportingV2Repository:
    ...
    def source_event_payloads_by_version(
        self,
        event_version_ids: Iterable[str],
    ) -> list[dict[str, Any]]:
        requested = {
            str(value).strip()
            for value in event_version_ids
        ...
        with self.engine.begin() as connection:
            rows = connection.execute(
                select(source_events).where(
                    source_events.c.event_version_id.in_(requested),
                    source_events.c.acceptance_state == "accepted",
                )
            ).mappings().all()
        found = {str(row["event_version_id"]) for row in rows}
        missing = requested - found
        if missing:
            raise ValueError(
                "accepted source event versions are missing: "
                + ", ".join(sorted(missing))
            )
        return [
            {
        ...
        for period_id, (start_date, end_date) in (
            ...
            results[period_id] = {
                ...
                "prequalification": self.record_metric_observation(
                    ...
                    publication_state="shadow",
                    unavailable_reason=(
                        "Completed At controls the monthly event period. An "
                        "exact immutable eligibility-period event is still "
                        "collecting, so no rate is published."
                    ),
                    as_of_at=as_of,
                ),
    ...
    def parallel_results(
        ...
        definition_version: str,
        *,
        period_starts: Iterable[str] = (),
    ) -> list[dict[str, Any]]:
        """Aggregate-only comparison evidence for acceptance review."""
        query = select(parallel_run_results).where(
            parallel_run_results.c.metric_id == str(metric_id).strip(),
            parallel_run_results.c.definition_version
            == str(definition_version).strip(),
        )
        starts = {str(value) for value in period_starts}
        if starts:
            query = query.where(parallel_run_results.c.period_start.in_(starts))
        query = query.order_by(parallel_run_results.c.recorded_at.desc())
        with self.engine.begin() as connection:
        ...
        return [
            {
                ...
                "period_start": row["period_start"],
                "period_end": row["period_end"],
                "variance_classification": row["variance_classification"],
                "unexplained_event_count": row["unexplained_event_count"],
                "unexplained_cents": row["unexplained_cents"],
                "acceptance_state": row["acceptance_state"],
                "evidence": json.loads(row["evidence_json"]),
                "recorded_at": row["recorded_at"].isoformat(),
