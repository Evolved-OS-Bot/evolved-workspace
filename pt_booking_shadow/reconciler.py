...
def reconcile_contact(
    ...
    cross_calendar_exact_matches: list[dict[str, str]] = []

    # Protect every appointment that exactly matches its own canonical slot
    # before considering broader same-week reschedules. Prefer the canonical
    # calendar, but treat the same contact, start and duration in any other
    # approved PT calendar as exact coverage. This captures trainer cover and
    # calendar reassignment without turning a calendar-specific absence into a
    # client-level gap.
        ...
        if exact is None:
            exact = next(
                (
                    event
                    for event in unmatched
                    if abs((event.start - expected_start).total_seconds()) <= 60
                    and abs(event.duration_minutes - expected_duration) <= 1
                ),
                None,
            )
            if exact is not None:
                expected_record = calendars[expected_calendar]
                actual_record = calendars[exact.calendar_id]
                cross_calendar_exact_matches.append(
                    {
                        "expected_start": expected_start.isoformat(),
                        "actual_appointment_id": exact.id,
                        "expected_calendar_id": expected_calendar,
                        "actual_calendar_id": exact.calendar_id,
                        "expected_trainer": expected_record.trainer,
                        "actual_trainer": actual_record.trainer,
                    }
                )
        if exact:
    ...
    finding.evidence.update(
        {
            ...
            "cross_calendar_exact_matches": cross_calendar_exact_matches,
